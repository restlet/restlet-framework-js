...
module.exports = {
  ...
  buildDistFiles: function(target, config, fn){
    ...
    util.each(files, function(filepath) {
    	...
    	src = currentThis.evaluateTemplateFile(target, src, config.baseSrc, (filename=='commons.js' || filename=='index.js'), currentModuleName, classModules);
  ...
  evaluateTemplateFile: function(target, src, baseSrc, commons, currentModuleName, classModules) {
    var currentThis = this;
    ...
    util.each(lines, function(line) {
      if (util.isIncludeLine(line)) {
        ...
        var includeContent = grunt.file.read((commons?'commons':'src') + '/' + includeFilepath);
        includeContent = currentThis.cleanJavaScriptContent(target, includeContent, currentModuleName, classModules, baseSrc);
  ...
  cleanJavaScriptContent: function(target, src, currentModuleName, classModules, baseSrc) {
    /*
     * // [ifndef nodejs]
    ...
    util.each(lines, function(line) {
      if (util.isIfndef(line)) {
        ...
        line = util.handleClassPattern(line, currentModuleName, classModules, baseSrc); 
        if (!uncomment) {
          newLines.push(line);
        } else {
