...
function checkMediaTypes(mediaTypes, supportedMediaType) {
    ...
    _.forEach(mediaTypes, function(mediaType) {
      console.log('- mediaType = '+mediaType+', supportedMediaType = '+supportedMediaType);
...
converters.push({
  ...
  toString: function(obj, callback) {
    callback(JSON.stringify(obj), 'application/json');
  }
});
...
converters.push({
  ...
  toString: function(obj, callback) {
    callback(xmlBuilder.buildObject(obj), 'application/xml');
  }
});
