...
server.createComponent = function() {
  ...
  return {
     ...
     * @param {Number|Object} port|configuration the port or the configuration object of the server to add
     * @component
     * @method
     * @api public
     */
...
server.createRouter = function() {
  ...
  return {
    ...
    attach: function() {
      ...
      var route = { next: next, path: path, pathRegexpString: pathRegexpString };
      routes.push(route);
    ...
    handle: function(request, response) {
      ...
      if (!_.isEmpty(routes)) {
        ...
        matchedRoutes = _.filter(routes, function(route) {
          ...
          debugRouter('Testing route with regexp ' + route.pathRegexpString + ' - match = ' + match);
          return match;
        });
      }
      ...
      debugRouter('Selected route ' + matchedRoutes[0].path + ' for path ' + path);
...
function isContainPayload(req) {
  return (req.entity != null && req.entity.mediaType);
}
...
function isExtensionMatch(request, configuredAccept) {
  var acceptedMediaTypes = request.acceptedMediaTypes;
...
server.createServerResource = function() {
  ...
  return {
    ...
    getJson: function() {
      ...
      debugServerResource('Registered handler for method GET (accept JSON content)');
    ...
    getXml: function() {
      ...
      debugServerResource('Registered handler for method GET (accept XML content)');
    ...
    getYaml: function() {
      ...
      debugServerResource('Registered handler for method GET (accept YAML content)');
    ...
    handle: function(request, response) {
      ...
      var matchedHandlers = _.filter(handlers, function(handler) {
        ...
        var match = false;
        // Check method matching if any specified
        if (configuration != null && configuration.method != null) {
          match = isMethodMatch(request, configuration.method);
        }

        // Check extension matching if any specified and if the method
        // matches
        if (match && configuration != null && configuration.accept != null) {
          match = isExtensionMatch(request, configuration.accept);
        }
