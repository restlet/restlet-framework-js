...
server.createRouter = function() {
  ...
  return {
    ...
    handle: function(request, response) {
      ...
      console.log('>> matchedRoutes = '+matchedRoutes.length);
...
function convertInputEntity(entity, request, response) {
  console.log('>> convertInputEntity - request.entity.mediaType = '+request.entity.mediaType.name);
  ...
  converter.toObject(entity.text, function(obj) {
    convertedObj = obj;
  });
