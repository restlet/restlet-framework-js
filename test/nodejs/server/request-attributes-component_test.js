...
exports['simple resource tests'] = {
  setUp: function(done) {
    ...
    var application = restlet.Application.create(function() {
      ...
      r1.addMethod("get", "json", function() {
    	...
    	var param1 = this.getRequestAttributes()['param1'];
    	var param2 = this.getRequestAttributes()['param2'];
      ...
      router.attach("/{param1}/test/{param2}", r1);
      return router;
    });
  ...
  'get with json and simple attributes': function (test) {
    var options = utils.createRequestOptions('localhost:8000/param1/test/param2?p1=10&p2=11','GET',{});
    utils.executeHttpRequest(test, options, null, function(res) {
      test.equal(200, res.statusCode);
      test.equal('37', res.headers['content-length']);
      ...
      res.on('end', function() {
        test.equal('{\"param1\":"param1",\"param2\":\"param2\"}', data.join(''));
        test.done();
      });
    });
  },
